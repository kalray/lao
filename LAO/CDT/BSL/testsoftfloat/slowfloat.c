...
int8_t slow_float_rounding_mode;
int8_t slow_float_exception_flags;
int8_t slow_float_detect_tininess;
#ifdef FLOATX80
int8_t slow_floatx80_rounding_precision;
#endif
...
//static void print_floatx(FILE *f, floatX a) {
//  if(a.isNaN) {
//    fprintf(f,"NAN");
//    return;
//  }
...
//    return;
//  }
//  fprintf(f,"%x.%llx-%llx",a.exp,a.sig.a1,a.sig.a0);
//}

static bits128X shortShift128Left( bits128X a, int8_t shiftCount )
{
    int8_t negShiftCount;
...
static bits128X shortShift128RightJamming( bits128X a, int8_t shiftCount )
{
    int8_t negShiftCount;
    bits64 extra;
...
static floatX int32ToFloatX( int32_t a )
{
    floatX ax;
...
static int32_t floatXToInt32( floatX ax )
{
    int8_t savedExceptionFlags;
    int32_t shiftCount;
    int32_t z;
...
static floatX int64ToFloatX( int64_t a )
{
//    uint64_t absA;
    floatX ax;
...
static int64_t floatXToInt64( floatX ax )
{
    int8_t savedExceptionFlags;
    int32_t shiftCount;
    int64_t z;
...
static float32 floatXToFloat32( floatX zx )
{
    ...
    if ( zx.isZero ) { ret = zx.sign ? 0x80000000 : 0; return ret; }
    if ( zx.isInf ) { ret = zx.sign ? 0xFF800000 : 0x7F800000; return ret; }
    if ( zx.isNaN ) { ret = 0xFFFFFFFF; return ret; }
...
float32 slow_int32_to_float32( int32_t a )
{
    return floatXToFloat32( int32ToFloatX( a ) );

}

float64 slow_int32_to_float64( int32_t a )
{
...
floatx80 slow_int32_to_floatx80( int32_t a )
{
...
float128 slow_int32_to_float128( int32_t a )
{
...
float32 slow_int64_to_float32( int64_t a )
{
...
float64 slow_int64_to_float64( int64_t a )
{
...
floatx80 slow_int64_to_floatx80( int64_t a )
{
...
float128 slow_int64_to_float128( int64_t a )
{
...
int32_t slow_float32_to_int32( float32 a )
{
...
int32_t slow_float32_to_int32_round_to_zero( float32 a )
{
    int8_t savedRoundingMode;
    int32_t z;
...
int64_t slow_float32_to_int64( float32 a )
{
...
int64_t slow_float32_to_int64_round_to_zero( float32 a )
{
    int8_t savedRoundingMode;
    int64_t z;
...
float32 slow_float32_max( float32 a, float32 b )
{
    floatX ax, bx;

    ax = float32ToFloatX( a );
    bx = float32ToFloatX( b );
    // Fixes #1557, we don't raise invalid exception for min/max so we
    // always return the signaling NaN. 
    ...
    return ( !( ax.isNaN || bx.isNaN ) && floatXLe( bx, ax ) ) ||  bx.isNaN ? a : b  ;
}

float32 slow_float32_min( float32 a, float32 b )
{
    floatX ax, bx;
    ...
    return ( !( ax.isNaN || bx.isNaN ) && floatXLe( ax, bx ) ) ||  bx.isNaN ? a : b  ;
}
...
int32_t slow_float64_to_int32( float64 a )
{
...
int32_t slow_float64_to_int32_round_to_zero( float64 a )
{
    int8_t savedRoundingMode;
    int32_t z;
...
int64_t slow_float64_to_int64( float64 a )
{
...
int64_t slow_float64_to_int64_round_to_zero( float64 a )
{
    int8_t savedRoundingMode;
    int64_t z;
...
float64 slow_float64_max( float64 a, float64 b )
{
    floatX ax, bx;
    ...
    if (((a == 0x8000000000000000ULL) && (b == 0)) || 
        ((b == 0x8000000000000000ULL) && (a == 0))) {
            return 0;
    }
...
float64 slow_float64_min( float64 a, float64 b )
{
    floatX ax, bx;
...
int32_t slow_floatx80_to_int32( floatx80 a )
{
...
int32_t slow_floatx80_to_int32_round_to_zero( floatx80 a )
{
    int8_t savedRoundingMode;
    int32_t z;
...
int64_t slow_floatx80_to_int64( floatx80 a )
{
...
int64_t slow_floatx80_to_int64_round_to_zero( floatx80 a )
{
    int8_t savedRoundingMode;
    int64_t z;
...
int32_t slow_float128_to_int32( float128 a )
{
...
int32_t slow_float128_to_int32_round_to_zero( float128 a )
{
    int8_t savedRoundingMode;
    int32_t z;
...
int64_t slow_float128_to_int64( float128 a )
{
...
int64_t slow_float128_to_int64_round_to_zero( float128 a )
{
    int8_t savedRoundingMode;
    int64_t z;
...
static bits64X shortShift64Left( bits64X a, int8_t shiftCount )
{
    int8_t negShiftCount;
...
static bits64X shortShift64RightJamming( bits64X a, int8_t shiftCount )
{
    int8_t negShiftCount;
    bits32 extra;
...
{
    floatX ax;
    // FIXME: commented the next line but don't understand why it was
    // present.
//    return 0;
